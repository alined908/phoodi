...
class MeetupListView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        coords = [request.GET.get('latitude', 34.052235), request.GET.get('longitude', -118.243683),request.GET.get("radius", 25)]
        today = date.today()
        ...
        if meetup_type == "public":
            ...
            meetups = Meetup.get_private(categories, coords, request, start, end)
        else:
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            member = MeetupMember.objects.create(user=user, meetup=meetup, admin=True)
            return Response(
                {
                    "status": "Success",
                    "meetup": MeetupSerializer(meetup, context={"user": user}).data,
...
class MeetupMembersView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": "Meetup does not exist"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": "Meetup does not exist"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        if member.user == meetup.creator:
            return Response(
                {"error": "Creator cannot leave meetup"},
