        ...
        return Response(
            {
                "status": "Success",
                "uri": chat_room.uri,
                "message": "New chat sessions created",
            }
        )
...
class ChatRoomView(APIView):
    ...
    def patch(self, request, *args, **kwargs):
        ...
        chat_room.members.get_or_create(user=user, room=chat_room)

        members = [
            UserSerializer(member.user).data for member in chat_room.members.all()
        ]
...
class ChatRoomMessageView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get("last", False):
            messages = ChatRoomMessage.objects.raw(
                "SELECT * FROM meetup_chatroommessage AS c WHERE c.room_id=%s AND c.id<%s ORDER BY c.id DESC LIMIT 50",
                [chat_room.id, request.GET.get("last")],
            )
        else:
            messages = ChatRoomMessage.objects.raw(
                "SELECT * FROM meetup_chatroommessage AS c WHERE c.room_id=%s ORDER BY c.id DESC LIMIT 50",
                [chat_room.id],
            )
