...
from meetup.serializers import MessageSerializer, MeetupEventSerializer, MeetupEventOptionSerializer, MeetupMemberSerializer
from channels.db import database_sync_to_async
from channels.layers import get_channel_layer
from django.test import Client
from django.contrib.auth.hashers import make_password
from meetup.models import ChatRoom, User, ChatRoomMessage, Meetup, MeetupEvent, MeetupMember
...
def create_user(email):
    user = User.objects.create(email=email, first_name="First", last_name="Name", password=make_password("password"))
    return user

@database_sync_to_async
def create_meetup(user, public):
    meetup = Meetup.objects.create(
        name="meetup-1", date=datetime.date.today(), 
        location="Berkeley", latitude=34.228754, 
        longitude=-118.2351192, public=public
    )
...
def create_event(meetup, member, random=True):
    event = MeetupEvent.objects.create(
        meetup=meetup, creator=member,
        title="event", distance=10000,
        price="1,2", start=now(),
        end=now(), random=random,
        entries={}
    )
    return event
...
class TestWebsockets:
    ...
    async def test_user_notifications(self, settings):
        ...
        assert response['message'] == {
            "chat_message": 0, "friend_inv": 0, 
            "meetup_inv": 0, "meetup": 0, "friend": 0
        }
    ...
    async def test_meetup_consumer_vote_meetup_event(self, settings):
        ...
        assert response['message']['option'] == json.loads(json.dumps(option_json[option.id]))
