...
class MeetupTest(TestCase):
    ...
    def test_email(self):
        ...
        self.assertEqual(
            mail.outbox[0].subject, "%s has been finalized." % (self.private.name,)
        )
...
class MeetupEventTest(TestCase):
    ...
    def test_MeetupEventsView_PATCH_valid(self):
        response = client.patch(
            "/api/meetups/" + self.meetup.uri + "/events/" + str(self.event.id) + "/",
            data=json.dumps(self.valid_payload, default=str),
            content_type="application/json",
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
...
class MeetupMemberTest(TestCase):
    ...
    def test_MeetupMembersView_POST_invalid_meetup(self):
        response = client.post(
            "/api/meetups/" + "gibberish" + "/members/", {"email": "daniel@gmail.com"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, {"error": "Meetup does not exist"})
    ...
    def test_MeetupMembersView_DELETE_invalid_meetup(self):
        response = client.delete(
            "/api/meetups/" + "gibberish" + "/members/", {"email": "test@gmail.com"}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.data, {"error": "Meetup does not exist"})
    ...
    def test_MeetupMembersView_DELETE_invalid_member(self):
        response = client.delete(
            "/api/meetups/" + self.meetup.uri + "/members/", {"email": self.user2.email}
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class MeetupEmailViewTest(TestCase):
    ...
    def test_MeetupEmailView_POST_valid(self):
        ...
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(
            mail.outbox[0].subject, "%s has been finalized." % (self.meetup.name,)
        )
