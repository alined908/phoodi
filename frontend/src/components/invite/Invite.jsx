...
class Invite extends Component {
  ...
  handleClick = async (inv, newStatus, type) => {
    type === inviteType.meetup
      ? this.props.respondMeetupInvite(
          inv.meetup.uri,
          inv.uri,
          newStatus,
          this.props.pos
    ...
    return (
      <div className={`${styles.invite} elevate-0`}>
        ...
        {!this.state.responded && inv.status === 1 && (
          <div>
            <Button
              ...
              color="primary"
              onClick={() =>
                this.handleClick(this.props.inv, 2, this.props.type)
              }
            >
              Confirm
            </Button>
            <Button
              ...
              color="secondary"
              onClick={() =>
                this.handleClick(this.props.inv, 3, this.props.type)
              }
            >
              Delete
            </Button>
        ...
        {this.state.responded &&
          this.state.status === 2 &&
          this.props.type === inviteType.friend && (
            <span>
              <Link to={`/profile/${inv.sender.id}`}>
        ...
        {this.state.responded &&
          this.state.status === 2 &&
          this.props.type === inviteType.meetup && (
            <span>
