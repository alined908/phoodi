    ...
    handlePasswordChange = async (values) => {
        try {
            const response = await axiosClient.post(
                `/auth/users/set_password/`, values, {headers: {
        ...
        return (
            <form onSubmit={handleSubmit(this.handlePasswordChange)}>
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <Field name="current_password" type="password" component={renderTextField} label="Current Password"/>
                    </Grid>
                    <Grid item xs={12}>
                        <Field name="new_password" type="password" component={renderTextField} label="New Password"/>
                    </Grid> 
                    <Grid item xs={12}>
                        <Field name="re_new_password" type="password" component={renderTextField} label="Confirm Password"/>
                    </Grid> 
