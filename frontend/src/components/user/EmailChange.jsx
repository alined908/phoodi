    ...
    handleEmailChange = async (values) => {
        try {
            const response = await axiosClient.post(
                `/auth/users/set_email/`, values, {headers: {
        ...
        return (
            <form onSubmit={handleSubmit(this.handleEmailChange)}>
                <Grid container spacing={2}>
                    <Grid item xs={12}>
                        <Field name="new_email" type="email" component={renderTextField} label="New Email"/>
                    </Grid>
                    <Grid item xs={12}>
                        <Field name="re_new_email" type="email" component={renderTextField} label="Confirm New Email"/>
                    </Grid> 
                    <Grid item xs={12}>
                        <Field name="current_password" type="password" component={renderTextField} label="Password"/>
                    </Grid> 
