...
describe("Meetup Reducer", () => {
  ...
  it("should handle ADD_MEETUP_MEMBER", () => {
    ...
    const newMeetup = {
      ...
      [meetup.uri.uri]: {
        ...meetup.uri,
        members: { ...meetup.uri.members, ...meetupMember3 },
      },
    };
  ...
  it("should handle ADD_MEETUP_EVENT", () => {
    ...
    const newEvent = {
      ...meetup,
      [meetup.uri.uri]: { ...meetup.uri, events: { ...events } },
    };
    const expectedState = { ...defaultState, meetups: { ...newEvent } };
    expect(newState).toEqual(expectedState);
  });
  ...
  it("should handle EDIT_MEETUP_EVENT", () => {
    ...
    const state = {
      ...
      meetups: {
        ...meetup,
        [meetup.uri.uri]: { ...meetup.uri, events: { ...event } },
      },
    };
    const newState = meetupReducer(state, action);
    const newEvent = {
  ...
  it("should handle DELETE_MEETUP_EVENT", () => {
    ...
    const newEvent = {
      ...meetup,
      [meetup.uri.uri]: { ...meetup.uri, events: {} },
    };
    const expectedState = { ...defaultState, meetups: { ...newEvent } };
    expect(newState).toEqual(expectedState);
  });
  ...
  it("should handle ADD_EVENT_OPTION", () => {
    ...
    const newOption = {
      ...
      [meetup.uri.uri]: {
        ...meetup.uri,
        events: { ...events, [1]: { ...event[1], options: { ...options } } },
      },
    };
  ...
  it("should handle VOTE_EVENT_OPTION", () => {
    const action = {
      ...
      payload: {
        meetup: meetup.uri.uri,
        event_id: 1,
        option_id: 1,
        option: option[1],
        member: meetupMember1,
      },
    };
    const newEvent = {
      ...meetup,
    ...
    const newOption = {
      ...
      [meetup.uri.uri]: {
        ...
        events: {
          ...events,
          [1]: { ...event[1], options: { ...options, ...option } },
        },
      },
    };
    const expectedState = { ...defaultState, meetups: { ...newOption } };
  ...
  it("should handle DELETE_EVENT_OPTION", () => {
    ...
    const state = {
      ...defaultState,
      meetups: { ...meetup, [meetup.uri.uri]: { ...meetup.uri, events } },
    };
    const newState = meetupReducer(state, action);
    const newEvent = {
      ...meetup,
