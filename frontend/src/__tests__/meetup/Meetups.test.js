...
describe("Meetups tests", () => {
    ...
    it('shows loading state if meetup is fetching', () => {
        const newProps = {...props, isMeetupsFetching: true, isMeetupsInitialized: false}
    ...
    it('calls for meetups if date range is different', () => {
        ...
        expect(picker.instance().props.onDatesChange({startDate: moment(), endDate: moment()}))
        expect(props.getMeetups.mock.calls.length).toBe(2)
    })
    ...
    it ('rejects date outside of range', () => {
        ...
        expect(wrapper.instance().isOutsideRange(moment().subtract(1, 'days'))).toBe(true)
    })
})