...
describe("ChatWindow unit ", () => {
    ...
    it ('render loader if fetching state' , () => {
        const fetching = {...props, isMessagesInitialized: false, isMessagesFetching: true}
    ...
    it ("shows link to user if room is meetup" , () => {
        const meetup = {...props, room: mocks.rooms.meetup, activeRoom: mocks.rooms.meetup.uri}
    ...
    it ('calls getMoreMessages if isMoreRetrievable', () => {   
        ...
        const retrievable = {...props, isMoreRetrievable: true, getMoreMessages: jest.fn()}
        const wrapper = mount(
            <Router>
        ...
        const event = {target: {scrollTop: 0, scrollHeight: 50, clientHeight: 60}, persist: jest.fn()}
    ...
    it ('sets bound state if bottom reached', async () => {
        ...
        const event2 = {target: {scrollTop: 50, scrollHeight: 50, clientHeight: 0}, persist: jest.fn()}
