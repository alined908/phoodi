...
export const getUserFriendInvites = () => async (dispatch) => {
  ...
  try {
    const response = await axiosClient.get("/api/friends/invite/", {
      headers: {
...
export const getUserMeetupInvites = () => async (dispatch) => {
  ...
  try {
    const response = await axiosClient.get("/api/meetups/invite/", {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
...
export const respondMeetupInvite = (roomURI, inviteURI, status) => async (
  dispatch
) => {
  try {
    const response = await axiosClient.patch(
