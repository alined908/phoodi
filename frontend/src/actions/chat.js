...
export const getRoom = (uri) => async (dispatch) => {
  ...
  try {
    const response = await axiosClient.get(`/api/chats/${uri}/`, {
      headers: {
    ...
    dispatch({
      ...
      payload: "Unable to get chat room.",
    });
  }
};
...
export const getRooms = () => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      type: types.GET_ROOMS_ERROR,
      payload: "Unable to get chat rooms.",
    });
  }
};
...
export const getMessages = (room) => async (dispatch) => {
  ...
  try {
    const response = await axiosClient.get(`/api/chats/${room}/messages/`, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
    ...
    dispatch({
      ...
      payload: "Unable to get messages.",
    });
  }
};
...
export const getMoreMessages = (room, last_message_id) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: "Unable to get more messages.",
    });
  }
};
