...
export const signup = (formProps, redirectOnSuccess) => async (dispatch) => {
  try {
    const response = await axiosClient.post("/auth/users/", formProps, {
    ...
    dispatch({
      ...
      payload:
        "Your account has been registered. Complete signup by activating account in email.",
    });
...
export const refreshToken = (dispatch) => {
  ...
  var freshTokenPromise = axiosClient
    ...
    .then((res) => {
      ...
      dispatch({
        type: types.AUTH_USER,
        payload: { access: res.data.access, user: decoded.user },
      });
...
export const addSettings = (data) => async (dispatch) => {
  try {
    const response = await axiosClient.post("/api/users/settings/", data, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
    });
...
export const addPreference = (data, user_id) => async (dispatch) => {
  try {
    ...
    return Promise.all([
      dispatch({ type: types.ADD_PREFERENCE, payload: response.data }),
      dispatch({
        type: types.ADD_GLOBAL_MESSAGE,
...
export const editPreference = (data, user_id, pref_id) => async (dispatch) => {
  try {
    const response = await axiosClient.patch(
      `/api/users/${user_id}/preferences/${pref_id}/`,
      data,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
...
export const deletePreference = (user_id, category_id) => async (dispatch) => {
  try {
    const response = await axiosClient.delete(
      `/api/users/${user_id}/preferences/${category_id}/`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
        },
